...
describe('SearchProvider and CommandMenu', () => {
  ...
  it('renders the command palette when the palette is open', async () => {
    ...
    await expect
      .element(screen.getByRole('option', { name: /^Sistema$/ }))
      .toBeInTheDocument()
    await expect.element(getByText('Painel')).toBeInTheDocument()
  })
  ...
  it('navigates to a top-level route and closes the palette when a nav item is selected', async () => {
    ...
    await userEvent.click(screen.getByText('Vagas'))

    expect(mocks.navigate).toHaveBeenCalledWith({ to: '/vagas' })
    await expect
      .element(screen.getByPlaceholder(COMMAND_MENU_PLACEHOLDER))
      .not.toBeInTheDocument()
  })
  ...
  it('navigates for nested sidebar items (group with sub-items)', async () => {
    ...
    await userEvent.click(
      getByRole('option', { name: 'Configurações Aparência' })
    )
