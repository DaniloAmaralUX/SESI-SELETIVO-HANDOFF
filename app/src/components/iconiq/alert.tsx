...
export function resolveAlertToastStackOffset({
  ...
  const offset = stackIndex * (ALERT_TOAST_STACK_HEIGHT + ALERT_TOAST_STACK_GAP)
...
export interface AlertProps
  extends Omit<MotionDivProps, 'title'>, VariantProps<typeof alertVariants> {
  children?: ReactNode
...
function AlertDismissButton({
  ...
  return (
    <motion.button
      ...
      className={cn(
        'supports-[corner-shape:squircle]:corner-squircle relative -my-2 -mr-2 inline-flex size-10 shrink-0 items-center justify-center self-start rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none supports-[corner-shape:squircle]:rounded-[11px]',
...
export const Alert = forwardRef<HTMLDivElement, AlertProps>(function Alert(
  ...
  const containerVariants = getContainerVariants(resolvedPosition, reduceMotion)
