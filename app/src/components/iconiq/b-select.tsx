...
const selectTriggerClassName = cn(
  ...
  'flex min-h-11 w-full touch-manipulation items-center justify-between gap-2 border border-[color:var(--sel-border)] bg-[color:var(--sel-surface)] px-4 py-3 text-left text-sm font-medium text-[color:var(--sel-foreground)] transition-[background-color,color,box-shadow] duration-200 ease-[cubic-bezier(0.22,1,0.36,1)] hover:bg-accent/72 focus:outline-none focus-visible:ring-2 focus-visible:ring-[color:color-mix(in_oklch,var(--sel-ring),transparent_50%)] disabled:cursor-not-allowed disabled:opacity-50 data-placeholder:text-[color:var(--sel-muted-foreground)]'
)
...
const selectItemClassName = cn(
  controlCornerClassName,
  'group relative isolate flex min-h-11 cursor-pointer touch-manipulation items-center gap-3 py-2.5 pr-8 pl-3 text-sm text-[color:var(--sel-foreground)] outline-none select-none data-[disabled]:pointer-events-none data-[disabled]:cursor-not-allowed data-[disabled]:text-[color:var(--sel-muted-foreground)] data-[disabled]:opacity-50'
)
...
function SelectValue({
  ...
  const valueClassName = typeof className === 'function' ? undefined : className
  ...
  return (
    ...
    >
      ...
      {description ? (
        <p
          className={cn(
            'text-xs leading-snug tracking-tight text-pretty text-muted-foreground',
            descriptionClassName
          )}
          id={descriptionId}
        >
...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.GroupLabel
      className={cn(
        'px-3 pb-1.5 text-[10px] font-medium tracking-[0.16em] text-[color:var(--sel-muted-foreground)] uppercase',
        className
      )}
