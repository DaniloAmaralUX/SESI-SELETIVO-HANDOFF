...
function FileUploadDropzone({
  ...
  return (
    <motion.div
      ...
      className={[
        'bg-paper relative overflow-hidden border px-5 py-4',
    ...
    >
      ...
      <div className='pointer-events-none relative flex items-center gap-4'>
        ...
        <div className='min-w-0 flex-1'>
          ...
          <p className='truncate text-xs text-muted-foreground' id={hintId}>
            {dropzoneDescription}
          </p>
        </div>
  ...
  return (
    <div className='relative h-11 w-11 shrink-0'>
      ...
      <motion.div
        ...
        className={[
          'bg-paper absolute inset-0 border border-border',
          controlCornerClassName,
        ].join(' ')}
        transition={shellTransition}
      />
...
function FileUploadBrowseHint({
  ...
  if (prefersReducedMotion) {
    return (
      <div className='inline-flex shrink-0 items-center gap-1 text-[10px] tracking-[0.18em] text-foreground/70 uppercase'>
  ...
  return (
    <motion.div
      className='inline-flex shrink-0 items-center gap-1 text-[10px] tracking-[0.18em] text-foreground/70 uppercase'
...
export const FileUpload = forwardRef<HTMLDivElement, FileUploadProps>(
  function FileUpload(
    ...
    const canSimulateUpload = shouldSimulateUpload && !isControlled && !onUpload
    const atMaxFiles =
    ...
    return (
      ...
      >
        {description ? (
          <p className='mb-2 text-sm text-muted-foreground' id={descriptionId}>
            {description}
          </p>
        ) : null}
    ...
    <div className='relative z-10 flex items-center gap-1'>
      {item.status === 'error' ? (
        <button
          ...
          className={[
            'flex h-8 w-8 items-center justify-center text-muted-foreground transition-colors hover:bg-accent/60 hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring/30 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50',
            controlCornerClassName,
          ].join(' ')}
          disabled={disabled}
          onClick={(event) => {
