...
const panelClass = cn(
  corner,
  'z-[300] max-w-[calc(100vw-1.5rem)] min-w-[12rem] overflow-hidden border border-[color:color-mix(in_oklch,var(--dd-border),transparent_40%)] bg-[color:var(--dd-surface)] shadow-[0_14px_34px_-22px_color-mix(in_oklch,var(--dd-foreground),transparent_72%)] outline-none'
)

const itemClass = cn(
  corner,
  'relative flex min-h-11 w-full cursor-default scroll-m-1 items-center justify-between gap-3 px-3 py-2.5 text-left text-sm text-[color:var(--dd-foreground)] transition-colors outline-none select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50'
)
...
export interface DropdownValueProps extends React.HTMLAttributes<HTMLSpanElement> {
  placeholder?: string
}
...
export interface DropdownSubTriggerProps extends React.ComponentPropsWithoutRef<
...
export interface DropdownSubContentProps extends React.ComponentPropsWithoutRef<
...
>(({ className, ...props }, ref) => (
  <span
    className={cn(
      'ml-auto pl-3 text-[0.68rem] tracking-[0.12em] text-[color:var(--dd-muted-foreground)]',
      className
    )}
    ref={ref}
    {...props}
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.Label
    className={cn(
      'px-3 pt-1 pb-1 text-[0.68rem] font-medium tracking-[0.12em] text-[color:color-mix(in_oklch,var(--dd-muted-foreground),transparent_20%)] uppercase',
      className
    )}
    ref={ref}
    {...props}
