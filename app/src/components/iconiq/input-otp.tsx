...
function useOTPLength() {
  ...
  if (import.meta.env.MODE !== 'production' && length === null) {
...
function OTPSlotPlaceholderHint({
  ...
  if (prefersReducedMotion) {
    return (
      <span
        aria-hidden
        className='pointer-events-none text-muted-foreground/50 select-none'
      >
        {hint}
      </span>
    )
...
const OTPSlot = forwardRef<HTMLInputElement, OTPSlotProps>(
  ({ index: _index, className, placeholder, ...props }, ref) => {
    ...
    if (import.meta.env.MODE !== 'production' && _index !== undefined) {
      console.warn(
