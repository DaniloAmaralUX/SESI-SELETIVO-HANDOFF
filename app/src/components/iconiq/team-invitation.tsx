...
export function TeamInvitationTabs({
  ...
  return (
    <>
      <div className='mt-5 border-t border-foreground/8' />
...
export function TeamInvitationTab({
  ...
  return (
    <motion.button
      ...
      className={cn(
        'relative flex h-9 shrink-0 items-center overflow-hidden rounded-full text-[13px] font-medium transition-colors duration-300',
        isActive
    ...
    >
      ...
      <AnimatePresence initial={false} mode='popLayout'>
        {isActive ? (
          <motion.span
            animate={{ opacity: 1 }}
            className={cn('pr-3.5 whitespace-nowrap', icon ? '' : 'pl-3.5')}
            exit={{ opacity: 0 }}
...
export function TeamInvitationSectionLabel({
  ...
  return (
    <h3 className={cn('text-[17px] font-medium text-foreground', className)}>
      {children}
    </h3>
  )
}
  ...
  return (
    <div className={cn('mt-5', className)}>
      ...
      <div className='mt-3 flex gap-3'>
        ...
        <motion.button
          className='shrink-0 rounded-xl bg-primary px-4 py-3 text-[14px] font-medium text-primary-foreground'
          onClick={invite}
          type='button'
          whileTap={reduceMotion ? undefined : { scale: 0.96 }}
        >
...
function MemberAvatar({ member }: { member: TeamMember }) {
  ...
  return (
    <span className='flex size-12 shrink-0 items-center justify-center rounded-xl border border-foreground/8 bg-muted text-sm font-medium text-foreground'>
...
function EmptyRow({ children }: { children: ReactNode }) {
  ...
  return (
    <motion.li
      animate={{ opacity: 1 }}
      className='rounded-xl border border-dashed border-foreground/8 px-4 py-3 text-[13px] text-muted-foreground'
      initial={reduceMotion ? false : { opacity: 0 }}
      layout
    >
      {children}
