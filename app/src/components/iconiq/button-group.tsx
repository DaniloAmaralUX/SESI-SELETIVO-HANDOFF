    ...
    return (
      <MotionButton
        className={cn(
          ...
          'relative cursor-pointer touch-manipulation overflow-hidden select-none',
          className
        )}
        data-slot='button'
        disabled={disabled}
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const showStandaloneBorder = !connected && variant !== 'ghost' && showBorder
    ...
    props: mergeProps<'div'>(
      {
        className: cn(
          ...
          "flex items-center gap-2 font-medium text-muted-foreground [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4",
          connected
    ...
    >
      ...
      {childArray.map((child, index) => {
        ...
        return (
          <RippleButton
            className={cn(
              'relative inline-flex items-center justify-center py-0 font-medium whitespace-nowrap text-muted-foreground transition-colors hover:text-foreground',
              ...
              showDividers && !isLast && 'border-r border-border',
              childClassName
            )}
            contentClassName={contentSizeStyles[resolvedSize]}
            disableRipple={disableRipple}
...
function SegmentedControl({
  ...
  const moveByDirection = (index: number, direction: 1 | -1) => {
    const nextIndex = getEnabledOptionIndex(normalizedOptions, index, direction)
        ...
        {normalizedOptions.map((option, index) => {
          ...
          return (
            <motion.button
              ...
              className={cn(
                'relative z-10 inline-flex cursor-pointer touch-manipulation items-center justify-center rounded-md py-0 font-medium whitespace-nowrap outline-none select-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset',
