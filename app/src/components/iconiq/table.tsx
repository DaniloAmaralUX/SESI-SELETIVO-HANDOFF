...
function getTableRowClassName({
  ...
  return cn(
    'grid items-center',
    isHeader
      ? 'border-y border-border text-xs tracking-wider text-muted-foreground uppercase'
      : cn(
...
export interface TableSelectHeadProps extends Omit<
...
export interface TableSelectCellProps extends Omit<
...
>(({ className, ...props }, ref) => (
  <caption
    className={cn('pt-6 text-xs text-muted-foreground', className)}
...
const TableEmpty = React.forwardRef<HTMLTableRowElement, TableEmptyProps>(
  (
    ...
    return (
      ...
      >
        <td
          className='col-span-full px-2 py-16 text-center text-sm text-muted-foreground'
    ...
    return (
      <button
        ...
        className={cn(
          ...
          'flex min-h-10 w-full items-center gap-2 rounded-md px-3 py-2 text-left transition-colors transition-shadow outline-none focus-visible:ring-2 focus-visible:ring-foreground/20 focus-visible:ring-offset-2 focus-visible:ring-offset-background',
          active
            ? 'text-foreground'
