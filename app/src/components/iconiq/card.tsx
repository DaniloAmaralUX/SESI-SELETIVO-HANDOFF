...
function getCardSurfaceClassName({
  ...
  return cn(
    ...
    'relative flex transform-gpu flex-col gap-4 overflow-hidden border border-border/70 bg-card/95 py-4 text-sm text-card-foreground backdrop-blur-[2px]',
    ...
    'has-data-[slot=card-footer]:pb-0 has-[>[data-slot=card-image]:first-child]:pt-0 has-[>img:first-child]:pt-0',
    '[&_[data-slot=card-image]_img]:rounded-none [&>img]:rounded-none',
...
function CardTitle({ as = 'h3', className, ...props }: CardTitleProps) {
  ...
  return (
    <MotionTitle
      className={cn(
        'cn-font-heading text-base leading-snug font-medium tracking-[-0.01em]',
        className
      )}
...
function CardFooter({ className, ...props }: CardSectionProps) {
  ...
  return (
    <MotionDiv
      className={cn(
        'flex items-center rounded-b-[inherit] border-t border-border/60 bg-muted/45 p-4 text-muted-foreground backdrop-blur-[1px] transition-colors duration-300 supports-[corner-shape:squircle]:[corner-shape:inherit]',
        className
      )}
