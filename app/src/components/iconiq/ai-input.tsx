  ...
  return (
    <div className='flex items-start justify-between gap-2.5 px-2.5 py-2'>
      <div className='min-w-0'>
        <p className='leading-tight font-medium text-foreground'>{label}</p>
        {description ? (
...
function DropdownActionItem({
  ...
  return (
    ...
    >
      ...
      {shortcut ? (
        <span className='relative z-10 shrink-0 text-xs text-muted-foreground tabular-nums'>
          {shortcut}
        </span>
      ) : null}
    </button>
  ...
  return (
    <>
      <button
        ...
        className='flex max-w-[calc(100%-3rem)] min-w-0 items-center gap-1.5 rounded-full py-1 text-sm transition-colors hover:text-foreground'
        onClick={toggleOpen}
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            animate={{ opacity: 1, scale: 1, y: 0 }}
            aria-label={ariaLabel}
            className={`absolute z-50 w-max max-w-[calc(100vw-2rem)] min-w-52 rounded-2xl border border-border bg-card p-1 shadow-[0_10px_28px_-18px_rgba(0,0,0,0.25)] ${align === 'end' ? 'right-0' : 'left-0'} ${upward ? `bottom-full mb-2 ${align === 'end' ? 'origin-bottom-right' : 'origin-bottom-left'}` : `top-full mt-2 ${align === 'end' ? 'origin-top-right' : 'origin-top-left'}`}`}
      ...
      <AnimatePresence>
        {open ? (
          <motion.div
            animate={{ opacity: 1, scale: 1, y: 0 }}
            aria-label='More options'
            className={`absolute left-0 z-50 w-max max-w-[calc(100vw-2rem)] min-w-64 overflow-hidden rounded-2xl border border-border bg-card p-1 shadow-[0_10px_28px_-18px_rgba(0,0,0,0.25)] ${upward ? 'bottom-full mb-2 origin-bottom-left' : 'top-full mt-2 origin-top-left'}`}
            exit={{ opacity: 0, scale: 0.97, y: upward ? 4 : -4 }}
            initial={{ opacity: 0, scale: 0.96, y: upward ? 6 : -6 }}
            onMouseLeave={() => setHoveredValue(null)}
            role='menu'
          ...
          >
            <AnimatePresence custom={slideDir} initial={false} mode='wait'>
              {activeParent ? (
                ...
                >
                  <button
                    className='relative isolate flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm font-medium'
                    onClick={closeSub}
        ...
        >
          <AnimatePresence initial={false}>
            {messages.map((message) => (
              <motion.div
                animate={{ opacity: 1, scale: 1, y: 0 }}
                className='max-w-[85%] self-end rounded-3xl rounded-br-lg bg-muted px-4 py-2.5 text-[15px] leading-relaxed break-words whitespace-pre-wrap text-foreground sm:text-sm'
        ...
        <div className='flex items-center gap-1.5 px-2.5 pt-1 pb-2.5 sm:gap-2 sm:px-3 sm:pb-3'>
          ...
          <div className='ml-auto flex shrink-0 items-center gap-1 sm:gap-1.5'>
            ...
            <button
              ...
              className='hidden size-8 shrink-0 items-center justify-center rounded-full text-foreground/60 transition-colors hover:text-foreground sm:size-9 @[21rem]:flex'
              onClick={onMicClick}
              type='button'
            >
