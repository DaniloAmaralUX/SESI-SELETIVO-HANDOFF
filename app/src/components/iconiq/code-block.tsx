...
function CopyButton({
  ...
  return (
    <motion.button
      ...
      className={cn(
        'flex h-7 items-center gap-1.5 rounded-md px-2 text-xs font-medium transition-colors duration-200 outline-none focus-visible:ring-2 focus-visible:ring-ring/60',
        floating &&
  ...
  return (
    ...
    >
      {hasTab ? (
        ...
        <div className='flex items-end justify-between gap-3 border-b border-border/70 bg-gradient-to-b from-muted/70 to-muted/40 pt-2 pr-1.5 pl-2.5 dark:from-white/[0.05] dark:to-white/[0.02]'>
          <div className='relative -mb-px flex min-w-0 items-center gap-2 rounded-t-lg border border-b-0 border-border/70 bg-background px-3.5 py-1.5 dark:bg-zinc-950'>
      ...
      <div className='relative'>
        ...
        <div
          className='overflow-auto [scrollbar-width:thin] [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border/80 [&::-webkit-scrollbar-track]:bg-transparent'
          style={{ maxHeight }}
        >
                  ...
                  >
                    {showLineNumbers ? (
                      ...
                      <span
                        aria-hidden
                        className={cn(
                          'sticky left-0 z-10 mr-4 inline-block w-11 border-r border-border/60 pr-3 text-right text-xs leading-6 tabular-nums select-none',
                          isHighlighted
      ...
      <div className='flex items-center justify-between gap-3 border-t border-border/70 bg-muted/60 px-4 py-1.5 text-[11px] text-muted-foreground dark:bg-white/[0.04]'>
        <div className='flex min-w-0 items-center gap-2'>
          {language ? (
            <>
              <span className='truncate font-mono tracking-wider uppercase'>
                {language}
              </span>
