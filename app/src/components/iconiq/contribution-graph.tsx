...
const BlockTooltipStateContext = createContext<BlockTooltipPayload | null>(null)
  ...
  return createPortal(
    ...
    >
      <div className='relative rounded-lg border border-white/10 bg-zinc-950 px-2.5 py-1 text-white shadow-[0_8px_26px_-8px_rgba(0,0,0,0.55)] dark:border-black/10 dark:bg-zinc-100 dark:text-zinc-950'>
        {payload.content ?? (
          <div className='flex items-baseline gap-1.5 text-[11px] leading-none whitespace-nowrap'>
        ...
        <span
          aria-hidden
          className='absolute top-full left-1/2 -mt-px -translate-x-1/2 border-x-[5px] border-t-[6px] border-x-transparent border-t-zinc-950 dark:border-t-zinc-100'
        />
      </div>
    </div>,
    document.body
...
export const ContributionGraph = ({
  ...
  if (fetched.status === 'error') {
    return (
      <div
        className={cn(
          'flex w-max max-w-full items-center gap-2 rounded-lg border border-dashed border-border px-4 py-3 text-sm text-muted-foreground',
          className
        )}
  ...
  return (
    <div
      className={cn(
        ...
        '[&::-webkit-scrollbar]:h-1 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border hover:[&::-webkit-scrollbar-thumb]:bg-muted-foreground/40 [&::-webkit-scrollbar-track]:bg-transparent',
        className
      )}
    ...
    >
      ...
      {new Array(maxLevel + 1).fill(undefined).map((_, level) =>
        children ? (
          ...
          >
            <rect
              className={cn('stroke-border stroke-[1px]', LEVEL_CLASSES)}
              data-level={level}
              height={blockSize}
              rx={blockRadius}
              ry={blockRadius}
