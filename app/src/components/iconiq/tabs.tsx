...
>(({ children, className, fullWidth = false, loop = true, ...props }, ref) => {
  ...
  return (
    <TabsPrimitive.List
      className={cn(
        'relative isolate no-scrollbar max-w-full gap-1',
        fullWidth ? 'flex w-full' : 'inline-flex w-fit',
        isVertical
          ? 'flex-col overflow-x-hidden overflow-y-auto overscroll-y-contain'
          : 'flex-row overflow-x-auto overflow-y-hidden overscroll-x-contain',
        variant === 'pill'
          ? 'rounded-2xl bg-muted p-2'
          : "px-1 whitespace-nowrap after:pointer-events-none after:absolute after:inset-x-0 after:bottom-0 after:z-0 after:h-px after:bg-border/50 after:content-['']",
        className
      )}
      data-slot='tabs-list'
      loop={loop}
  ...
  (
    ...
    return (
      <TabsPrimitive.Trigger
        ...
        className={cn(
          'relative z-10 inline-flex min-h-9 min-w-0 shrink-0 touch-manipulation items-center justify-center gap-1.5 rounded-lg px-3 py-1.5 text-sm font-medium whitespace-nowrap transition-colors outline-none disabled:pointer-events-none disabled:opacity-50',
          variant === 'pill'
            ? cn(
      ...
      >
        ...
        {badge ? (
          <span
            className={cn(
              'inline-flex min-w-5 shrink-0 items-center justify-center rounded-full px-1.5 py-0.5 text-[10px] font-medium tabular-nums',
...
>(({ children, className, value: contentValue, ...props }, ref) => {
  ...
  return (
    <TabsPrimitive.Content
      className={cn(
        'relative ring-offset-background outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2',
