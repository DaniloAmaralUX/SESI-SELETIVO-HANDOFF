...
const popoverTriggerClassName = cn(
  'flex h-10 items-center gap-2.5 border border-border bg-card px-3 text-left text-sm text-foreground transition-colors hover:border-foreground/30 focus-visible:ring-2 focus-visible:ring-[color:var(--color-ring)] focus-visible:outline-none',
  colorPickerShellWidthClassName,
  controlCornerClassName
)

const swatchTriggerClassName = cn(
  'inline-flex shrink-0 transition-transform hover:scale-105 focus-visible:ring-2 focus-visible:ring-[color:var(--color-ring)] focus-visible:outline-none',
  controlCornerClassName
)
...
function oklchToRgb(lightness: number, chroma: number, hue: number) {
  ...
  const shortPrime = lightness - 0.089_484_177_5 * axisA - 1.291_485_548 * axisB
  const linearRed =
...
>) {
  ...
  const applyColorState = useCallback(
    (
      ...
      const resolvedAlpha = showAlphaRef.current ? clampPercent(nextAlpha) : 100
        ...
        return (
          <li key={`${preset}-${index}`}>
            <button
              ...
              className={cn(
                'transition-transform hover:scale-105 focus-visible:ring-2 focus-visible:ring-[color:var(--color-ring)] focus-visible:outline-none',
                controlCornerClassName
              )}
              disabled={disabled}
    ...
    >
      ...
      <div className='relative'>
        ...
        <div
          ...
          className={cn(
            'relative h-40 w-full cursor-crosshair touch-none overflow-hidden transition-[background-color] duration-300 select-none focus-visible:ring-2 focus-visible:ring-[color:var(--color-ring)] focus-visible:outline-none',
            controlCornerClassName
          )}
          id={svFieldId}
        ...
        <div className='relative z-20 mt-4 flex items-center justify-between text-sm text-muted-foreground'>
                ...
                <div className='flex items-center gap-2'>
                  ...
                  <div className='flex min-w-0 flex-1 items-center gap-0.5'>
                    ...
                    <ColorPickerTextInput
                      ...
                      className={cn(
                        'w-full min-w-0 bg-transparent text-base font-light tracking-wide outline-none',
  ...
  return (
    <div className='flex min-w-0 flex-1 flex-col items-center gap-0.5'>
      <div className='flex w-full min-w-0 items-baseline justify-center gap-0.5'>
        <ColorPickerTextInput
          aria-label={label}
          className='w-full min-w-0 bg-transparent text-center text-base font-light tabular-nums outline-none disabled:cursor-not-allowed disabled:opacity-50'
          disabled={disabled}
          inputMode='decimal'
          onBlur={() => {
            setFocused(false)
