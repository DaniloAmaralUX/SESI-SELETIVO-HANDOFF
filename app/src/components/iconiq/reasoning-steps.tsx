...
interface ReasoningStepsProps extends Omit<
...
function ReasoningStepDetails({
  ...
  return (
    ...
    >
      <CollapsiblePrimitive.Trigger
        className='flex items-center gap-1.5 rounded-md px-1 py-1 text-[12px] text-muted-foreground transition-colors hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none'
        render={(triggerProps) => (
      ...
      <MorphPanel
        ...
        reduceMotion={reduceMotion}
      >
        <div className='flex flex-col gap-1 text-[12px] leading-5 text-muted-foreground'>
          {children}
        </div>
      </MorphPanel>
    </CollapsiblePrimitive.Root>
