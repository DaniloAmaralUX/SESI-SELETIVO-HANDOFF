  ...
  return (
    <p
      className={cn(
        ...
        'group/wall max-w-3xl text-justify text-base leading-[1.75] font-medium tracking-[-0.01em] motion-reduce:[--testimonial-blur:0px] sm:text-xl',
        className
      )}
      style={
        {
...
function TestimonialAvatar({
  ...
  if (avatar && !failed) {
    return (
      ...
      <img
        alt=''
        className='mr-[0.35em] inline-block size-[1.15em] rounded-full border border-border/60 object-cover align-[-0.2em] select-none'
        height={24}
  ...
  return (
    <span
      aria-hidden
      className='mr-[0.35em] inline-flex size-[1.15em] items-center justify-center rounded-full border border-border bg-muted align-[-0.2em] text-[0.45em] font-semibold tracking-[0.08em] text-muted-foreground uppercase select-none'
    >
      {getInitials(name)}
    </span>
  )
  ...
  return (
    <span
      className={cn(
        ...
        'group/quote relative -mx-[0.15em] -my-[0.3em] inline cursor-pointer px-[0.15em] py-[0.3em] [filter:blur(0px)] outline-none [transition:filter_0.3s_cubic-bezier(0.32,0.72,0,1),opacity_0.2s_ease-out,color_0.3s_ease] motion-reduce:transition-none',
    ...
    >
      ...
      <span className='inline-block w-0 overflow-visible align-middle'>
        <span
          className={cn(
            'pointer-events-none inline-flex translate-y-1 flex-col gap-[0.3em] pl-[0.5em] font-mono text-[0.52em] leading-none tracking-[0.14em] whitespace-nowrap uppercase opacity-0 blur-[2px] transition-[opacity,transform,filter] duration-300 ease-out motion-reduce:translate-y-0 motion-reduce:transition-none',
