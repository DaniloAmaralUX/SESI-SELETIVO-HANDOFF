...
// Neutralizado para o app SESI: os tokens vêm do tema do projeto (theme.css/
// index.css), nunca da paleta --ic-* embutida do registry iconiq.
const componentThemeClassName = ''

export const _paletaOriginalIconiq =
...
export function Tabs({
  ...
  const activeContent =
    resolvedValue === undefined
      ? contentElements[0]
      : contentElements.find((element) => element.props.value === resolvedValue)
  ...
  ({ children, className, onMouseLeave, ...props }, ref) => {
    ...
    return (
      <TabsPrimitive.List
        {...props}
        className={cn(
          "relative isolate no-scrollbar inline-flex max-w-full items-center overflow-x-auto overscroll-x-contain whitespace-nowrap after:pointer-events-none after:absolute after:inset-x-0 after:bottom-0 after:z-0 after:h-px after:bg-border/50 after:content-['']",
          className
        )}
        loop
        onMouseLeave={(event) => {
  ...
  (
    ...
    return (
      <TabsPrimitive.Trigger
        ...
        className={cn(
          // shrink-0: com a lista em overflow-x, sem ele os triggers encolhem
          // abaixo do conteúdo e os rótulos se sobrepõem no mobile
          'relative inline-flex min-h-11 min-w-11 shrink-0 touch-manipulation items-center justify-center rounded-md px-6 py-4 text-sm tracking-tight outline-none focus-visible:ring-2 focus-visible:ring-ring/40 focus-visible:ring-inset',
          className
        )}
